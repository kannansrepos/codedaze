...
const RecentPost = () => {
  ...
  // const [markdownData, setMarkdownData] = useState<never[]>([]);
  ...
  const getMarkdownDataList = async (markdownFileNames: string[]) => {
    const markdownDataList = await fetch('/api/markdown', {
      method: 'POST',
      body: JSON.stringify({
        markdownFileNames,
      }),
    });
    if (!markdownDataList.ok) {
      console.log('Failed to fetch markdown data');
    }
    const response = await markdownDataList.json();
  ...
  useEffect(() => {
    const fetchData = async () => {
      const postList = [
        'unlocking_ai_power_a_deep_dive_into_semantic_kernel_and_its_key_components',
      ];
      const dataList = await getMarkdownDataList(postList);
